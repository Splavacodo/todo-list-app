...
export class ProjectController {
    ...
    setupEditTaskFormEventListeners() {
        const taskDescription: HTMLTextAreaElement = document.querySelector('#task-description');
        const MAX_ROWS = 10;
        ...
        const taskTitle: HTMLInputElement = document.querySelector("#main-task-name");
        const editTaskBtn: HTMLButtonElement = document.querySelector(".form-edit-task-btn");

        taskTitle.addEventListener("input", () => {
            (taskTitle.value === "") ? editTaskBtn.toggleAttribute("disabled") : editTaskBtn.removeAttribute("disabled");
        });
        ...
        const mainEditTaskForm: HTMLFormElement = document.querySelector(".main-edit-task-form");
        const formULParent: HTMLUListElement = (mainEditTaskForm.parentNode as HTMLUListElement);
        const ulParentId: string = formULParent.dataset["parentId"];

        if (this.sectionService.containsId(ulParentId)) {
            const parentSection: Section = this.sectionService.getSection(ulParentId);
    ...
    getEditTaskForm(parentId: string): HTMLFormElement {
        return ProjectView.getEditTaskForm(parentId);
    }
}
